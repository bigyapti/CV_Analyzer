...
client = MongoClient('mongodb+srv://bbigyapti:FYRJT4MtkNkMRGEu@cluster0.fekxhl2.mongodb.net/?retryWrites=true&w=majority')
...
def prediction():
    ...
    name,email,phone,url = entities_from_pdf(original_text)
    ...
    return render_template('prediction.html', skills=skills, score=score, predicted_category_name=predicted_category_name, url=url, email=email, phone=phone,name=name)
...
def entities_from_pdf(pdf_text):
    # Define patterns for matching entities
    pattern_name = re.compile(r'\b[A-Z][a-z]+ [A-Z][a-z]+\b')
    pattern_email = re.compile(r'\b[\w\.-]+@[\w\.-]+\b')
    pattern_phone = re.compile(r'\b\d{3}-\d{3}-\d{4}\b')
    pattern_url = re.compile(r'\b(?:http|https)://\S+\b')

    # Search for matches for entities
    name = re.search(pattern_name, pdf_text)
    email = re.search(pattern_email, pdf_text)
    phone = re.search(pattern_phone, pdf_text)
    url = re.search(pattern_url, pdf_text)
